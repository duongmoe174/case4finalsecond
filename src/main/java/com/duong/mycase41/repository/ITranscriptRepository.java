...
public interface ITranscriptRepository extends PagingAndSortingRepository<Transcript, Long> {
     ...
     @Query(nativeQuery = true, value = "SELECT full_name as 'nameStudent',point1 as 'pointone',point2 as 'pointtwo',asbj.name as 'nameSubject' FROM transcript t join student s  on s.id = t.student_id join appsubject asbj on t.app_subject_id = asbj.id where full_name like ?1")
